    ...
    def process_record(self, row):
        ...
        try:
            ...
            record = IncomingFundTransferRecord(
                ...
                reportingDateTimestamp=safe_string(row[3]),             # ORDER_TIMESTAMP
                transferChannel=safe_string(row[4]),                    # 'EFT'
                subCategoryTransferChannel=safe_string(row[5]) if row[5] else None,  # NULL
                recipientName=safe_string(row[6]),                      # TRIM(TRIM(cust.FIRST_NAME) || ' ' || ...)
                senderAccountNumber=safe_string(row[7]) if row[7] else None,         # NULL
                recipientIdentificationType=safe_string(row[8]),        # CASE WHEN UPPER(TRIM(id.ISSUE_AUTHORITY)) = 'NIDA'...
                recipientIdentificationNumber=safe_string(row[9]),      # id.ID_NO
                recipientCountry=safe_string(row[10]),                  # 'TANZANIA, UNITED REPUBLIC OF'
                senderName=safe_string(row[11]),                        # 'Bank Of Tanzania'
                senderBankOrFspCode=safe_string(row[12]),               # 'TANZTZTXXXX'
                senderAccountOrWalletNumber=safe_string(row[13]),       # im.BENEF_IBAN_ACC
                serviceCategory=safe_string(row[14]),                   # 'Mobile Banking Transactions'
                serviceSubCategory=safe_string(row[15]),                # 'Inter-Bank'
                currency=safe_string(row[16]),                          # curr.SHORT_DESCR
                orgAmount=safe_string(row[17]),                         # ORDER_AMOUNT
                usdAmount=safe_string(row[18]),                         # USD conversion
                tzsAmount=safe_string(row[19]),                         # TZS conversion
                senderInstruction=safe_string(row[20]),                 # REMITTANCE_INFO
