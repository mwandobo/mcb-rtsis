#!/usr/bin/env python3
"""
Interbank Loan Payable Streaming Pipeline - Producer and Consumer run simultaneously
"""
...
from processors.interbank_loan_payable_processor import InterbankLoanPayableProcessor, InterbankLoanPayableRecord

class InterbankLoanPayableStreamingPipeline:
    def __init__(self, batch_size=50):  # Increased from 5 to 50 for better performance
        self.config = Config()
        self.db2_conn = DB2Connection()
        self.processor = InterbankLoanPayableProcessor()
        ...
        self.logger.info("Interbank Loan Payable STREAMING Pipeline initialized")
    ...
    def get_interbank_loan_payable_query(self, last_eom_date=None, last_account_number=None):
        """Get the interbank loan payable query with cursor-based pagination for better performance"""
        
        # Use cursor-based pagination instead of ROW_NUMBER() for much better performance
        where_clause = "WHERE la.BOOK_BALANCE IS NOT NULL AND la.BOOK_BALANCE > 0"
        
        if last_eom_date and last_account_number:
            where_clause += f"""
            AND (la.EOM_DATE > '{last_eom_date}' 
                 OR (la.EOM_DATE = '{last_eom_date}' AND la.ACCOUNT_NUMBER > '{last_account_number}'))
            """
        
        query = f"""
        SELECT
            CURRENT_TIMESTAMP AS reportingDate,
            la.CUSTOMER_NAME  AS lenderName,
            la.ACCOUNT_NUMBER AS accountNumber,
            'TANZANIA, UNITED REPUBLIC OF' AS lenderCountry,
            la.LOAN_TYPE_NAME AS borrowingType,
            la.LAST_TRANSACTION_DATE AS transactionDate,
            la.DRAWDOWN_FST_DT AS disbursementDate,
            la.ACC_EXP_DT AS maturityDate,
            la.CURRENCY AS currency,

            /* ================= OPENING ================= */
            DECIMAL(COALESCE(la.BOOK_BALANCE, 0), 15, 2) AS orgAmountOpening,

            DECIMAL(
                CASE la.CURRENCY
                    WHEN 'USD' THEN COALESCE(la.BOOK_BALANCE, 0)
                    WHEN 'TZS' THEN COALESCE(la.BOOK_BALANCE, 0) / 2531.00
                    ELSE 0
                END,
                15, 2
            ) AS usdAmountOpening,
            ...
            DECIMAL(
                CASE la.CURRENCY
                    WHEN 'TZS' THEN COALESCE(la.BOOK_BALANCE, 0)
                    WHEN 'USD' THEN COALESCE(la.BOOK_BALANCE, 0) * 2531.00
                    ELSE 0
                END,
                15, 2
            ) AS tzsAmountOpening,
            ...
            /* ================= REPAYMENT ================= */
            DECIMAL(0, 15, 2) AS orgAmountRepayment,
            DECIMAL(0, 15, 2) AS usdAmountRepayment,
            DECIMAL(0, 15, 2) AS tzsAmountRepayment,

            /* ================= CLOSING ================= */
            DECIMAL(la.BOOK_BALANCE, 15, 2) AS orgAmountClosing,

            DECIMAL(
                CASE la.CURRENCY
                    WHEN 'USD' THEN la.BOOK_BALANCE
                    WHEN 'TZS' THEN la.BOOK_BALANCE / 2531.00
                    ELSE 0
                END,
                15, 2
            ) AS usdAmountClosing,
            ...
            DECIMAL(
                CASE la.CURRENCY
                    WHEN 'TZS' THEN la.BOOK_BALANCE
                    WHEN 'USD' THEN la.BOOK_BALANCE * 2531.00
                    WHEN 'EUR' THEN la.BOOK_BALANCE * (1.17 * 2531.00)
                    ELSE 0
                END,
                15, 2
            ) AS tzsAmountClosing,
            ...
            /* ================= TENURE ================= */
            CASE 
                WHEN la.ACC_EXP_DT IS NOT NULL AND la.DRAWDOWN_FST_DT IS NOT NULL
                THEN DAYS(la.ACC_EXP_DT) - DAYS(la.DRAWDOWN_FST_DT)
                ELSE 0
            END AS tenureDays,

            la.FINAL_INTEREST AS annualInterestRate,

            /* ================= INTEREST TYPE ================= */
            CASE
                WHEN la.FINAL_INTEREST IS NOT NULL
                     AND la.FINAL_INTEREST <> la.SELECTED_NORMAL_RA
                    THEN 'effective'
                WHEN la.SPREAD IS NOT NULL
                     AND la.SPREAD <> 0
                    THEN 'negotiated'
                ELSE 'nominal'
        ...
        {where_clause}
        ORDER BY la.EOM_DATE ASC, la.ACCOUNT_NUMBER ASC
    ...
    def get_total_count_query(self):
        ...
        FROM W_EOM_LOAN_ACCOUNT la
        WHERE la.BOOK_BALANCE IS NOT NULL
          AND la.BOOK_BALANCE > 0
        """
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for interbank loan payable"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            # Declare queue with durability
            channel.queue_declare(queue='interbank_loan_payable_queue', durable=True)
            
            connection.close()
            self.logger.info("RabbitMQ queue 'interbank_loan_payable_queue' setup complete")
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total interbank loan payable records available: {self.total_available:,}")
            ...
            while True:
                ...
                for attempt in range(self.max_retries):
                    try:
                        with self.db2_conn.get_connection() as conn:
                            cursor = conn.cursor()
                            batch_query = self.get_interbank_loan_payable_query(last_eom_date, last_account_number)
                ...
                for row in rows:
                    ...
                    last_eom_date = row[-2]  # EOM_DATE
                    last_account_number = row[-1]  # ACCOUNT_NUMBER
                    
                    # Remove cursor fields before processing
                    row_without_cursor = row[:-2]
                    record = self.processor.process_record(row_without_cursor, 'interbankLoanPayable')
                        ...
                        for attempt in range(self.max_retries):
                            try:
                                channel.basic_publish(
                                    exchange='',
                                    routing_key='interbank_loan_payable_queue',
                                    body=message,
        ...
        try:
            ...
            def process_message(ch, method, properties, body):
                ...
                try:
                    ...
                    record = InterbankLoanPayableRecord(**record_data)
            ...
            # Set QoS for controlled processing (increased for better throughput)
            channel.basic_qos(prefetch_count=10)  # Increased from 3 to 10
            channel.basic_consume(queue='interbank_loan_payable_queue', on_message_callback=process_message)
    ...
    def run_streaming_pipeline(self):
        ...
        self.logger.info("Starting Interbank Loan Payable STREAMING pipeline...")
        ...
        try:
            ...
            self.logger.info(f"""
            ==========================================
            Interbank Loan Payable Pipeline Summary:
            ==========================================
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Interbank Loan Payable Streaming Pipeline')
    ...
    # Create pipeline
    pipeline = InterbankLoanPayableStreamingPipeline(batch_size=args.batch_size)
