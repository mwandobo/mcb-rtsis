#!/usr/bin/env python3
"""
Complaint Statistics Streaming Pipeline - Producer and Consumer run simultaneously
Based on complaint-statistics.sql query
"""
...
@dataclass
class ComplaintStatisticsRecord:
    """Data class for complaint statistics records based on complaint-statistics.sql"""
    reportingDate: str
    complainantName: str
    complainantMobile: str
    complaintType: str
    occurrenceDate: str
    complaintReportingDate: str
    closureDate: str
    ...
    usdAmount: str
    employeeId: str
    referredComplaints: str
    complaintStatus: str


class ComplaintStatisticsStreamingPipeline:
    def __init__(self, batch_size=1000, consumer_batch_size=100):
        ...
        self.logger.info("Complaint Statistics STREAMING Pipeline initialized")
    ...
    def get_complaint_statistics_query(self):
        """Get the complaint statistics query from complaint-statistics.sql"""
        sql_file_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            'sqls', 'complaint-statistics.sql'
        )
    ...
    def get_total_count(self):
        """Get approximate total count of complaint statistics records from DB2"""
        try:
            with self.db2_conn.get_connection(log_connection=False) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM PROFITS.COMPLAINT_REGISTER")
                result = cursor.fetchone()
                count = result[0] if result else 0
                self.logger.info(f"Estimated record count from PROFITS.COMPLAINT_REGISTER: {count:,}")
                return count
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for complaint statistics with dead-letter exchange"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            # Declare dead-letter exchange and queue for failed messages
            channel.exchange_declare(exchange='complaint_statistics_dlx', exchange_type='direct', durable=True)
            channel.queue_declare(queue='complaint_statistics_dead_letter', durable=True)
            channel.queue_bind(
                queue='complaint_statistics_dead_letter',
                exchange='complaint_statistics_dlx',
                routing_key='complaint_statistics_queue'
            )
            
            # Declare main queue with dead-letter exchange routing
            try:
                channel.queue_declare(
                    queue='complaint_statistics_queue',
                    durable=True,
                    arguments={
                        'x-dead-letter-exchange': 'complaint_statistics_dlx',
                        'x-dead-letter-routing-key': 'complaint_statistics_queue'
                    }
                )
                ...
                self.logger.warning(
                    "Queue 'complaint_statistics_queue' already exists with different args. "
                    "Delete and recreate it to enable dead-letter support."
                )
                connection, channel = self.setup_rabbitmq_connection()
                channel.queue_declare(queue='complaint_statistics_queue', durable=True)
                self.logger.info("RabbitMQ queue 'complaint_statistics_queue' setup complete (without DLX)")
    ...
    def process_record(self, row):
        """Process a single complaint statistics record from DB2"""
        try:
            ...
            record = ComplaintStatisticsRecord(
                reportingDate=safe_string(row[0]),                      # reportingDate
                complainantName=safe_string(row[1]),                    # complainantName
                complainantMobile=safe_string(row[2]),                  # complainantMobile
                complaintType=safe_string(row[3]),                      # complaintType
                occurrenceDate=safe_string(row[4]),                     # occurrenceDate
                complaintReportingDate=safe_string(row[5]),             # complaintReportingDate
                closureDate=safe_string(row[6]),                        # closureDate
                agentName=safe_string(row[7]) if row[7] else None,      # agentName
                tillNumber=safe_string(row[8]) if row[8] else None,     # tillNumber
                currency=safe_string(row[9]),                           # currency
                tzsAmount=safe_string(row[10]),                         # tzsAmount
                orgAmount=safe_string(row[11]),                         # orgAmount
                usdAmount=safe_string(row[12]),                         # usdAmount
                employeeId=safe_string(row[13]),                        # employeeId
                referredComplaints=safe_string(row[14]),                # referredComplaints
                complaintStatus=safe_string(row[15])                    # complaintStatus
            )
            
            return record
            
        except Exception as e:
            self.logger.error(f"Error processing complaint statistics record: {e}")
            self.logger.error(f"Row data: {row}")
            self.logger.error(f"Row length: {len(row)}")
            raise
    
    def validate_record(self, record):
        """Validate complaint statistics record"""
        try:
            # Basic validation
            if not record.complainantName:
                self.logger.warning("Missing complainant name")
                return False
            
            if not record.complaintType:
                self.logger.warning("Missing complaint type")
                return False
            
            return True
            
        except Exception as e:
            self.logger.error(f"Error validating complaint statistics record: {e}")
            return False
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total complaint statistics records available: {self.total_available:,} (estimated)")
            ...
            query = self.get_complaint_statistics_query()
            self.logger.info("Executing complaint statistics query (single execution, streaming results)...")
    ...
    def consumer_thread(self):
        ...
        try:
            ...
            # Batch insert buffer
            insert_buffer: List[ComplaintStatisticsRecord] = []
            ...
            def process_message(ch, method, properties, body):
                ...
                try:
                    ...
                    record = ComplaintStatisticsRecord(**record_data)
            ...
            channel.basic_consume(queue='complaint_statistics_queue', on_message_callback=process_message)
            ...
            while not self.stop_consumer.is_set():
                try:
                    ...
                    if self.producer_finished.is_set():
                        ...
                        queue_state = channel.queue_declare(queue='complaint_statistics_queue', durable=True, passive=True)
    ...
    def insert_batch_to_postgres(self, records: List[ComplaintStatisticsRecord], pg_conn):
        """Batch insert complaint statistics records to PostgreSQL with duplicate prevention"""
        try:
            cursor = pg_conn.cursor()
            
            insert_query = """
            INSERT INTO "complaintStatistics" (
                "reportingDate", "complainantName", "complainantMobile", "complaintType",
                "occurrenceDate", "complaintReportingDate", "closureDate", "agentName",
                "tillNumber", "currency", "tzsAmount", "orgAmount", "usdAmount",
                "employeeId", "referredComplaints", "complaintStatus"
            ) VALUES %s
            ON CONFLICT ("complainantName", "occurrenceDate") DO NOTHING
            """
            
            values = [
                (
                    r.reportingDate, r.complainantName, r.complainantMobile, r.complaintType,
                    r.occurrenceDate, r.complaintReportingDate, r.closureDate, r.agentName,
                    r.tillNumber, r.currency, r.tzsAmount, r.orgAmount, r.usdAmount,
                    r.employeeId, r.referredComplaints, r.complaintStatus
                )
                for r in records
            ]
            ...
            self.logger.error(f"Error batch inserting {len(records)} complaint statistics records: {e}")
            raise
    
    def ensure_unique_index(self):
        """Ensure unique index on complainantName and occurrenceDate exists for ON CONFLICT duplicate prevention"""
        try:
            with self.get_postgres_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_complaintstatistics_unique
                    ON "complaintStatistics" ("complainantName", "occurrenceDate")
                """)
                conn.commit()
                self.logger.info("Unique index on complainantName and occurrenceDate verified/created")
        except Exception as e:
    ...
    def run_streaming_pipeline(self):
        ...
        self.logger.info("Starting Complaint Statistics STREAMING pipeline...")
        ...
        try:
            ...
            self.logger.info(f"""
            ==========================================
            Complaint Statistics Pipeline Summary:
            ==========================================
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Complaint Statistics Streaming Pipeline')
    ...
    # Create pipeline
    pipeline = ComplaintStatisticsStreamingPipeline(batch_size=args.batch_size, consumer_batch_size=args.consumer_batch_size)
