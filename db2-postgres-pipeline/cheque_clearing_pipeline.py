#!/usr/bin/env python3
"""
Cheque and Other Items for Clearing Pipeline - BOT Project
...
class ChequeClearingPipeline:
    def __init__(self, limit=None, start_date=None, resume=False):
        """
        Cheque and Other Items for Clearing Pipeline
        ...
        self.cheque_clearing_processor = ChequeClearingProcessor()
    ...
    def get_total_count(self):
        """Get total count of cheque clearing records"""
        try:
            with self.get_db2_connection() as conn:
                ...
                JOIN CUSTOMER ic ON da.FK_CUSTOMERCUST_ID = ic.CUST_ID
                JOIN CURRENCY cu ON cfc.FK_CURRENCYID_CURR = cu.ID_CURRENCY
                JOIN CUSTOMER pc ON cfc.FK_CUSTOMERCUST_ID = pc.CUST_ID
                LEFT JOIN BANK_BIC_LOOKUP bic
                   ON UPPER(TRIM(cfc.DRAWN_BANK)) = UPPER(TRIM(bic.BANK_NAME))
                JOIN PROFITS_ACCOUNT ppa ON ppa.CUST_ID = pc.CUST_ID
                """
                
                if self.start_date:
                    count_query += f" WHERE cfc.TRX_DATE >= DATE('{self.start_date}')"
    ...
    def run_complete_pipeline(self):
        """Run the complete cheque clearing pipeline"""
        self.logger.info("Starting Complete Cheque Clearing Pipeline")
        self.logger.info("=" * 60)
        ...
        try:
            ...
            self.logger.info(f"Getting total cheque clearing records from {self.start_date}...")
            total_records = self.get_total_count()
            self.logger.info(f"Total cheque clearing records available: {total_records}")
            
            if total_records == 0:
                self.logger.info("No cheque clearing records found")
                return
            
            # Step 3: Process using the SQL query from config
            self.logger.info("Executing cheque clearing query...")
            ...
            with self.get_db2_connection() as conn:
                ...
                if self.start_date:
                    ...
                    query = query.replace("ORDER BY", f" WHERE cfc.TRX_DATE >= DATE('{self.start_date}') ORDER BY")
                ...
                # Show sample data
                self.logger.info("Sample cheque clearing records:")
                for i, row in enumerate(rows[:min(3, len(rows))], 1):
                    self.logger.info(f"  {i}. Cheque: {row[1]} | Issuer: {row[2]} | Payee: {row[4]} | Amount: {row[15]}")
            ...
            with self.get_postgres_connection() as conn:
                ...
                for i, row in enumerate(rows, 1):
                    try:
                        ...
                        record = self.cheque_clearing_processor.process_record(row, self.table_config.name)
                        
                        if self.cheque_clearing_processor.validate_record(record):
                            # Use upsert which handles duplicates automatically
                            self.cheque_clearing_processor.insert_to_postgres(record, pg_cursor)
                            processed_count += 1
                            
                            if processed_count % 100 == 0:
                                self.logger.info(f"Processed {processed_count}/{len(rows)} records...")
                        else:
                            self.logger.warning(f"Invalid cheque clearing record skipped: {record.cheque_number}")
                            skipped_count += 1
            ...
            self.logger.info(f"CHEQUE CLEARING PIPELINE FINISHED!")
            ...
            with self.get_postgres_connection() as conn:
                ...
                cursor.execute(f"""
                    SELECT "chequeNumber", "issuerName", "payeeName", "orgAmountPayment", "currency"
                ...
                for record in sample_records:
                    cheque_number, issuer_name, payee_name, amount, currency = record
                    self.logger.info(f"  {cheque_number} | {issuer_name} | {payee_name} | {amount} {currency}")
...
def main():
    ...
    print("Complete Cheque Clearing Pipeline - BOT Project")
    print("=" * 60)
    
    # Enable resume mode by default for cheque clearing processing - starting from 2024
    pipeline = ChequeClearingPipeline(start_date='2024-01-01', resume=True)
