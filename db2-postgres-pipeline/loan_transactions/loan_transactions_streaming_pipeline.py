#!/usr/bin/env python3
"""
Loan Transactions Streaming Pipeline - Producer and Consumer run simultaneously
Based on loan-transaction.sql query
"""
...
class LoanTransactionRecord:
    """Data class for loan transaction records based on loan-transaction.sql"""
    reportingDate: str
    loanNumber: str
    transactionDate: str
...
class LoanTransactionsStreamingPipeline:
    def __init__(self, batch_size=1000, consumer_batch_size=100):
        ...
        self.logger.info("Loan Transactions STREAMING Pipeline initialized")
    ...
    def get_loan_transactions_query(self):
        """Get the loan transactions query from loan-transaction.sql"""
        sql_file_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            'sqls', 'loan-transaction.sql'
        )
    ...
    def get_total_count(self):
        """Get approximate total count of loan transaction records from DB2"""
        try:
            with self.db2_conn.get_connection(log_connection=False) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM GLI_TRX_EXTRACT WHERE PRF_ACCOUNT_NUMBER IS NOT NULL")
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for loan transactions with dead-letter exchange"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            # Declare dead-letter exchange and queue for failed messages
            channel.exchange_declare(exchange='loan_transactions_dlx', exchange_type='direct', durable=True)
            channel.queue_declare(queue='loan_transactions_dead_letter', durable=True)
            channel.queue_bind(
                queue='loan_transactions_dead_letter',
                exchange='loan_transactions_dlx',
                routing_key='loan_transactions_queue'
            )
            
            # Declare main queue with dead-letter exchange routing
            try:
                channel.queue_declare(
                    queue='loan_transactions_queue',
                    durable=True,
                    arguments={
                        'x-dead-letter-exchange': 'loan_transactions_dlx',
                        'x-dead-letter-routing-key': 'loan_transactions_queue'
                    }
                )
                ...
                self.logger.warning(
                    "Queue 'loan_transactions_queue' already exists with different args. "
                    "Delete and recreate it to enable dead-letter support."
                )
                connection, channel = self.setup_rabbitmq_connection()
                channel.queue_declare(queue='loan_transactions_queue', durable=True)
                self.logger.info("RabbitMQ queue 'loan_transactions_queue' setup complete (without DLX)")
    ...
    def process_record(self, row):
        """Process a single loan transaction record from DB2"""
        try:
            ...
            record = LoanTransactionRecord(
                reportingDate=safe_string(row[0]),
                loanNumber=safe_string(row[1]),
                transactionDate=safe_string(row[2]),
                loanTransactionType=safe_string(row[3]),
                loanTransactionSubType=safe_string(row[4]) if row[4] else None,
                currency=safe_string(row[5]),
                orgTransactionAmount=safe_string(row[6]),
                usdTransactionAmount=safe_string(row[7]) if row[7] else None,
                tzsTransactionAmount=safe_string(row[8])
            )
            
            return record
            
        except Exception as e:
            self.logger.error(f"Error processing loan transaction record: {e}")
            self.logger.error(f"Row data: {row}")
            self.logger.error(f"Row length: {len(row)}")
            raise
    
    def validate_record(self, record):
        """Validate loan transaction record"""
        try:
            # Basic validation
            ...
            self.logger.error(f"Error validating loan transaction record: {e}")
            return False
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total loan transaction records available: {self.total_available:,} (estimated)")
            ...
            query = self.get_loan_transactions_query()
            self.logger.info("Executing loan transactions query (single execution, streaming results)...")
    ...
    def consumer_thread(self):
        ...
        try:
            ...
            # Batch insert buffer
            insert_buffer: List[LoanTransactionRecord] = []
            ...
            channel.basic_consume(queue='loan_transactions_queue', on_message_callback=process_message)
    ...
    def insert_batch_to_postgres(self, records: List[LoanTransactionRecord], pg_conn):
        """Batch insert loan transaction records to PostgreSQL"""
        try:
            cursor = pg_conn.cursor()
            
            insert_query = """
            INSERT INTO "loanTransactions" (
                "reportingDate", "loanNumber", "transactionDate", "loanTransactionType",
                "loanTransactionSubType", "currency", "orgTransactionAmount",
                "usdTransactionAmount", "tzsTransactionAmount"
            ) VALUES %s
            """
            
            values = [
                (
                    r.reportingDate, r.loanNumber, r.transactionDate, r.loanTransactionType,
                    r.loanTransactionSubType, r.currency, r.orgTransactionAmount,
                    r.usdTransactionAmount, r.tzsTransactionAmount
                )
                for r in records
            ]
            ...
            self.logger.error(f"Error batch inserting {len(records)} loan transaction records: {e}")
            raise
    
    def run_streaming_pipeline(self):
        """Run the streaming pipeline with simultaneous producer and consumer"""
        self.logger.info("Starting Loan Transactions STREAMING pipeline...")
        ...
        try:
            ...
            self.logger.info(f"""
            ==========================================
            Loan Transactions Pipeline Summary:
            ==========================================
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Loan Transactions Streaming Pipeline')
    ...
    # Create pipeline
    pipeline = LoanTransactionsStreamingPipeline(batch_size=args.batch_size, consumer_batch_size=args.consumer_batch_size)
