#!/usr/bin/env python3
"""
Branch Streaming Pipeline - Producer and Consumer run simultaneously
Based on branch.sql query
"""
...
@dataclass
class BranchRecord:
    """Data class for branch records based on branch.sql"""
    reportingDate: str
    branchName: str
    ...
    altTelephoneNumber: Optional[str]
    branchCategory: str
    lastModified: Optional[str]


class BranchStreamingPipeline:
    def __init__(self, batch_size=1000, consumer_batch_size=100):
        ...
        self.logger.info("Branch STREAMING Pipeline initialized")
    ...
    def get_branch_query(self):
        """Get the branch query from branch.sql"""
    ...
    def get_total_count(self):
        """Get approximate total count of branch records from DB2"""
        try:
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for branch with dead-letter exchange"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            # Declare dead-letter exchange and queue for failed messages
            channel.exchange_declare(exchange='branch_dlx', exchange_type='direct', durable=True)
            channel.queue_declare(queue='branch_dead_letter', durable=True)
            channel.queue_bind(
                queue='branch_dead_letter',
                exchange='branch_dlx',
                routing_key='branch_queue'
            )
            ...
            try:
                channel.queue_declare(
                    ...
                    durable=True,
                    arguments={
                        'x-dead-letter-exchange': 'branch_dlx',
                        'x-dead-letter-routing-key': 'branch_queue'
                    }
                )
                self.logger.info("RabbitMQ queues setup complete (main + dead-letter)")
            except Exception:
                self.logger.warning(
                    "Queue 'branch_queue' already exists with different args. "
                    "Delete and recreate it to enable dead-letter support."
                )
                connection, channel = self.setup_rabbitmq_connection()
                channel.queue_declare(queue='branch_queue', durable=True)
                self.logger.info("RabbitMQ queue 'branch_queue' setup complete (without DLX)")
    ...
    def process_record(self, row):
        """Process a single branch record from DB2"""
        try:
            ...
            record = BranchRecord(
                ...
                branchName=safe_string(row[1]),
                taxIdentificationNumber=safe_string(row[2]) if row[2] else None,
                businessLicense=safe_string(row[3]) if row[3] else None,
                branchCode=safe_string(row[4]),
                qrFsrCode=safe_string(row[5]),
                region=safe_string(row[6]) if row[6] else None,
                district=safe_string(row[7]) if row[7] else None,
                ward=safe_string(row[8]) if row[8] else None,
                street=safe_string(row[9]) if row[9] else None,
                houseNumber=safe_string(row[10]) if row[10] else None,
                postalCode=safe_string(row[11]) if row[11] else None,
                gpsCoordinates=safe_string(row[12]),
                bankingServices=safe_string(row[13]),
                mobileMoneyServices=safe_string(row[14]),
                registrationDate=safe_string(row[15]),
                branchStatus=safe_string(row[16]),
                closureDate=safe_string(row[17]) if row[17] else None,
                contactPerson=safe_string(row[18]) if row[18] else None,
                telephoneNumber=safe_string(row[19]) if row[19] else None,
                altTelephoneNumber=safe_string(row[20]) if row[20] else None,
                branchCategory=safe_string(row[21]),
                lastModified=safe_string(row[22]) if row[22] else None
            )
            
            return record
            
        except Exception as e:
            self.logger.error(f"Error processing branch record: {e}")
    ...
    def validate_record(self, record):
        ...
        try:
            ...
            if not record.branchName:
                self.logger.warning("Missing branch name")
                return False
            
            return True
            
        except Exception as e:
            self.logger.error(f"Error validating branch record: {e}")
            return False
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total branch records available: {self.total_available:,} (estimated)")
            ...
            query = self.get_branch_query()
            self.logger.info("Executing branch query (single execution, streaming results)...")
    ...
    def consumer_thread(self):
        ...
        try:
            ...
            insert_buffer: List[BranchRecord] = []
            ...
            channel.basic_consume(queue='branch_queue', on_message_callback=process_message)
            ...
            while not self.stop_consumer.is_set():
                try:
                    ...
                    if self.producer_finished.is_set():
                        ...
                        queue_state = channel.queue_declare(queue='branch_queue', durable=True, passive=True)
    ...
    def insert_batch_to_postgres(self, records: List[BranchRecord], pg_conn):
        """Batch insert branch records to PostgreSQL with duplicate prevention"""
        try:
            ...
            values = [
                (
                    r.reportingDate, r.branchName, r.taxIdentificationNumber, r.businessLicense,
                    r.branchCode, r.qrFsrCode, r.region, r.district, r.ward, r.street,
                    r.houseNumber, r.postalCode, r.gpsCoordinates, r.bankingServices,
                    r.mobileMoneyServices, r.registrationDate, r.branchStatus, r.closureDate,
                    r.contactPerson, r.telephoneNumber, r.altTelephoneNumber, r.branchCategory,
                    r.lastModified
                )
                for r in records
            ]
            ...
            self.logger.error(f"Error batch inserting {len(records)} branch records: {e}")
            raise
    
    def ensure_unique_index(self):
        """Ensure unique index on branchCode exists for ON CONFLICT duplicate prevention"""
        try:
            with self.get_postgres_connection() as conn:
                ...
                """)
                conn.commit()
                self.logger.info("Unique index on branchCode verified/created")
        except Exception as e:
            self.logger.error(f"Failed to create unique index on branchCode: {e}")
            raise
    
    def run_streaming_pipeline(self):
        """Run the streaming pipeline with simultaneous producer and consumer"""
        self.logger.info("Starting Branch STREAMING pipeline...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Branch Streaming Pipeline')
    ...
    pipeline = BranchStreamingPipeline(batch_size=args.batch_size, consumer_batch_size=args.consumer_batch_size)
