...
def create_card_transactions_table():
    ...
    try:
        ...
        CREATE TABLE "cardTransactions" (
            ...
            "orgFacilitatorCommissionAmount" VARCHAR(50),
            "usdFacilitatorCommissionAmount" VARCHAR(50),
            "tzsFacilitatorCommissionAmount" VARCHAR(50),
            "currency" VARCHAR(10),
        ...
        indexes = [
            ...
            'CREATE UNIQUE INDEX idx_cardTransactions_transaction_id ON "cardTransactions"("transactionId")',
            'CREATE INDEX idx_cardTransactions_transaction_nature ON "cardTransactions"("transactionNature")',
            'CREATE INDEX idx_cardTransactions_reporting_date ON "cardTransactions"("reportingDate")',
            'CREATE INDEX idx_cardTransactions_beneficiary_name ON "cardTransactions"("beneficiaryName")',
        ...
        logger.info("-" * 80)
        logger.info(f"{'Column Name':<30} {'Data Type':<20} {'Max Length':<12} {'Nullable':<10}")
        logger.info("-" * 80)
        
        for col in columns:
            col_name, data_type, max_length, nullable = col
            max_len_str = str(max_length) if max_length else 'N/A'
            logger.info(f"{col_name:<30} {data_type:<20} {max_len_str:<12} {nullable:<10}")
