...
class OverdraftProcessor(BaseProcessor):
    ...
    def process_record(self, raw_data: Tuple, table_name: str) -> OverdraftRecord:
        ...
        return OverdraftRecord(
            ...
            timestamp_column_value=safe_str(raw_data[0]),  # reportingDate
            reporting_date=safe_str(raw_data[0]),
            account_number=safe_str(raw_data[1]),
            customer_identification_number=safe_str(raw_data[2]),
            client_name=safe_str(raw_data[3]),
            client_type=safe_str(raw_data[4]),
            borrower_country=safe_str(raw_data[5]),
            rating_status=safe_str(raw_data[6]) if raw_data[6] else None,
            cr_rating_borrower=safe_str(raw_data[7]) if raw_data[7] else None,
            grades_unrated_banks=safe_str(raw_data[8]) if raw_data[8] else None,
            group_code=safe_str(raw_data[9]) if raw_data[9] else None,
            related_entity_name=safe_str(raw_data[10]) if raw_data[10] else None,
            related_party=safe_str(raw_data[11]) if raw_data[11] else None,
            relationship_category=safe_str(raw_data[12]) if raw_data[12] else None,
            loan_product_type=safe_str(raw_data[13]),
            overdraft_economic_activity=safe_str(raw_data[14]),
            loan_phase=safe_str(raw_data[15]),
            transfer_status=safe_str(raw_data[16]),
            purpose_other_loans=safe_str(raw_data[17]),
            contract_date=safe_str(raw_data[18]),
            branch_code=safe_str(raw_data[19]),
            loan_officer=safe_str(raw_data[20]),
            loan_supervisor=safe_str(raw_data[21]) if raw_data[21] else None,
            currency=safe_str(raw_data[22]),
            org_sanctioned_amount=safe_float(raw_data[23]),
            usd_sanctioned_amount=safe_float(raw_data[24]) if raw_data[24] is not None else None,
            tzs_sanctioned_amount=safe_float(raw_data[25]),
            org_utilised_amount=safe_float(raw_data[26]),
            usd_utilised_amount=safe_float(raw_data[27]) if raw_data[27] is not None else None,
            tzs_utilised_amount=safe_float(raw_data[28]),
            org_cr_usage_last30_days_amount=safe_float(raw_data[29]),
            usd_cr_usage_last30_days_amount=safe_float(raw_data[30]) if raw_data[30] is not None else None,
            tzs_cr_usage_last30_days_amount=safe_float(raw_data[31]),
            disbursement_date=safe_str(raw_data[32]),
            expiry_date=safe_str(raw_data[33]),
            real_end_date=safe_str(raw_data[34]),
            org_outstanding_amount=safe_float(raw_data[35]),
            usd_outstanding_amount=safe_float(raw_data[36]) if raw_data[36] is not None else None,
            tzs_outstanding_amount=safe_float(raw_data[37]),
            latest_customer_credit_date=safe_str(raw_data[38]),
            latest_credit_amount=safe_float(raw_data[39]),
            prime_lending_rate=safe_float(raw_data[40]),
            annual_interest_rate=safe_float(raw_data[41]),
            collateral_pledged=safe_float(raw_data[42]),
            org_collateral_value=safe_float(raw_data[43]),
            usd_collateral_value=safe_float(raw_data[44]) if raw_data[44] is not None else None,
            tzs_collateral_value=safe_float(raw_data[45]),
            restructured_loans=safe_int(raw_data[46]),
            past_due_days=safe_int(raw_data[47]),
            past_due_amount=safe_float(raw_data[48]),
            org_accrued_interest_amount=safe_float(raw_data[49]),
            usd_accrued_interest_amount=safe_float(raw_data[50]) if raw_data[50] is not None else None,
            tzs_accrued_interest_amount=safe_float(raw_data[51]),
            org_penalty_charged_amount=safe_float(raw_data[52]),
            usd_penalty_charged_amount=safe_float(raw_data[53]) if raw_data[53] is not None else None,
            tzs_penalty_charged_amount=safe_float(raw_data[54]),
            org_loan_fees_charged_amount=safe_float(raw_data[55]),
            usd_loan_fees_charged_amount=safe_float(raw_data[56]) if raw_data[56] is not None else None,
            tzs_loan_fees_charged_amount=safe_float(raw_data[57]),
            org_loan_fees_paid_amount=safe_float(raw_data[58]),
            usd_loan_fees_paid_amount=safe_float(raw_data[59]) if raw_data[59] is not None else None,
            tzs_loan_fees_paid_amount=safe_float(raw_data[60]),
            org_tot_monthly_payment_amount=safe_float(raw_data[61]),
            usd_tot_monthly_payment_amount=safe_float(raw_data[62]) if raw_data[62] is not None else None,
            tzs_tot_monthly_payment_amount=safe_float(raw_data[63]),
            org_interest_paid_total=safe_float(raw_data[64]),
            usd_interest_paid_total=safe_float(raw_data[65]) if raw_data[65] is not None else None,
            tzs_interest_paid_total=safe_float(raw_data[66]),
            asset_classification_category=safe_str(raw_data[67]),
            sector_sna_classification=safe_str(raw_data[68]),
            neg_status_contract=safe_str(raw_data[69]),
            customer_role=safe_str(raw_data[70]),
            allowance_probable_loss=safe_float(raw_data[71]),
            bot_provision=safe_float(raw_data[72]),
