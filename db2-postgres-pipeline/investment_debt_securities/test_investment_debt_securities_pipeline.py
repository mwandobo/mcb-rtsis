#!/usr/bin/env python3
"""
Test Investment Debt Securities Pipeline
...
def test_db2_connection():
    ...
    try:
        ...
        with db2_conn.get_connection() as conn:
            ...
            logger.info(f"Found {count:,} investment debt securities records in DB2")
...
def test_postgresql_connection():
    ...
    try:
        ...
        if table_exists:
            ...
            logger.info(f"Table 'investmentDebtSecurities' exists with {count:,} records")
        else:
            logger.warning("Table 'investmentDebtSecurities' does not exist")
...
def test_rabbitmq_connection():
    ...
    try:
        ...
        channel.queue_declare(queue='test_investment_debt_securities', durable=True)
        logger.info("RabbitMQ connection successful")
        
        # Clean up test queue
        channel.queue_delete(queue='test_investment_debt_securities')
        connection.close()
...
def test_sql_query():
    ...
    try:
        pipeline = InvestmentDebtSecuritiesStreamingPipeline()
        query = pipeline.get_investment_debt_securities_query()
        
        # Test query with FETCH FIRST 5 ROWS ONLY
        test_query = query.rstrip(';') + " FETCH FIRST 5 ROWS ONLY"
        ...
        with pipeline.db2_conn.get_connection() as conn:
            ...
            if rows:
                ...
                logger.info(f"Sample record processed: Security Number = {record.securityNumber}, Issuer = {record.securityIssuerName}")
...
def run_mini_pipeline_test():
    ...
    try:
        ...
        pipeline = InvestmentDebtSecuritiesStreamingPipeline(batch_size=10, consumer_batch_size=5)
        ...
        query = pipeline.get_investment_debt_securities_query()
        test_query = query.rstrip(';') + " FETCH FIRST 10 ROWS ONLY"
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("INVESTMENT DEBT SECURITIES PIPELINE TEST SUITE")
    logger.info("=" * 60)
