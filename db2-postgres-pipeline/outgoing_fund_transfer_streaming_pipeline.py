...
from processors.outgoing_fund_transfer_processor import OutgoingFundTransferProcessor, OutgoingFundTransferRecord

class OutgoingFundTransferStreamingPipeline:
    def __init__(self, batch_size=50):
        self.config = Config()
        self.db2_conn = DB2Connection()
        self.processor = OutgoingFundTransferProcessor()
    ...
    def get_outgoing_fund_transfer_query(self, last_trn_date=None, last_trn_snum=None):
        """Get the outgoing fund transfer query with cursor-based pagination for better performance"""
        
        # Use cursor-based pagination instead of ROW_NUMBER() for much better performance
        where_clause = """WHERE gl.EXTERNAL_GLACCOUNT = '100007000'
  AND wdc.ID_NO IS NOT NULL"""
    ...
    def get_total_count_query(self):
        """Get total count of available outgoing fund transfer records"""
        return """
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for outgoing fund transfer"""
        try:
            ...
            connection.close()
            self.logger.info("RabbitMQ queue 'outgoing_fund_transfer_queue' setup complete")
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total outgoing fund transfer records available: {self.total_available:,}")
            ...
            while True:
                ...
                for attempt in range(self.max_retries):
                    try:
                        with self.db2_conn.get_connection() as conn:
                            cursor = conn.cursor()
                            batch_query = self.get_outgoing_fund_transfer_query(last_trn_date, last_trn_snum)
                ...
                for row in rows:
                    ...
                    record = self.processor.process_record(row_without_cursor, 'outgoing_fund_transfer')
...
def main():
    ...
    # Create pipeline
    pipeline = OutgoingFundTransferStreamingPipeline(batch_size=args.batch_size)
