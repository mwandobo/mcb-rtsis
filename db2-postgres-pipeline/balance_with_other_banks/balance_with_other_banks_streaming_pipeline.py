#!/usr/bin/env python3
"""
Balance with Other Banks Streaming Pipeline - Producer and Consumer run simultaneously
Based on balance-with-other-bank-v1.sql query
"""
...
class BalanceWithOtherBankRecord:
    """Data class for balance with other bank records based on balance-with-other-bank-v1.sql"""
    reportingDate: str
...
class BalanceWithOtherBanksStreamingPipeline:
    def __init__(self, batch_size=1000, consumer_batch_size=100):
        ...
        self.logger.info("Balance with Other Banks STREAMING Pipeline initialized")
    ...
    def get_balance_with_other_banks_query(self, last_timestamp=None):
        ...
        sql_file_path = os.path.join(
            ...
            'sqls', 'balance-with-other-bank-v1.sql'
        )
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for balance with other banks with dead-letter exchange"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            channel.exchange_declare(exchange='balance_with_other_banks_dlx', exchange_type='direct', durable=True)
            channel.queue_declare(queue='balance_with_other_banks_dead_letter', durable=True)
            channel.queue_bind(
                queue='balance_with_other_banks_dead_letter',
                exchange='balance_with_other_banks_dlx',
                routing_key='balance_with_other_banks_queue'
            )
            ...
            try:
                channel.queue_declare(
                    ...
                    durable=True,
                    arguments={
                        'x-dead-letter-exchange': 'balance_with_other_banks_dlx',
                        'x-dead-letter-routing-key': 'balance_with_other_banks_queue'
                    }
                )
                self.logger.info("RabbitMQ queues setup complete (main + dead-letter)")
            except Exception:
                self.logger.warning(
                    "Queue 'balance_with_other_banks_queue' already exists with different args. "
                ...
                self.logger.info("RabbitMQ queue 'balance_with_other_banks_queue' setup complete (without DLX)")
    ...
    def process_record(self, row):
        """Process a single balance with other bank record from DB2"""
        try:
            ...
            record = BalanceWithOtherBankRecord(
                reportingDate=safe_string(row[0]),
                accountNumber=safe_string(row[1]) if row[1] else None,
                accountName=safe_string(row[2]) if row[2] else None,
                bankCode=safe_string(row[3]) if row[3] else None,
                country=safe_string(row[4]),
                relationshipType=safe_string(row[5]),
                accountType=safe_string(row[6]),
                subAccountType=safe_string(row[7]),
                currency=safe_string(row[8]),
                orgAmount=safe_decimal(row[9]),
                usdAmount=safe_decimal(row[10]),
                tzsAmount=safe_decimal(row[11]),
                transactionDate=safe_string(row[12]) if row[12] else None,
                pastDueDays=safe_int(row[13]),
                allowanceProbableLoss=safe_int(row[14]),
                botProvision=safe_int(row[15]),
                assetsClassificationCategory=safe_string(row[16]),
                contractDate=safe_string(row[17]) if row[17] else None,
                maturityDate=safe_string(row[18]) if row[18] else None,
                externalRatingCorrespondentBank=safe_string(row[19]),
                gradesUnratedBanks=safe_string(row[20])
            )
            
            return record
            
        except Exception as e:
            self.logger.error(f"Error processing balance with other bank record: {e}")
            self.logger.error(f"Row data: {row}")
            self.logger.error(f"Row length: {len(row)}")
            raise
    
    def validate_record(self, record):
        """Validate balance with other bank record"""
        try:
            ...
            self.logger.error(f"Error validating balance with other bank record: {e}")
            return False
    ...
    def consumer_thread(self):
        ...
        try:
            ...
            insert_buffer: List[BalanceWithOtherBankRecord] = []
            ...
            channel.basic_consume(queue='balance_with_other_banks_queue', on_message_callback=process_message)
            ...
            while not self.stop_consumer.is_set():
                try:
                    ...
                    if self.producer_finished.is_set():
                        ...
                        queue_state = channel.queue_declare(queue='balance_with_other_banks_queue', durable=True, passive=True)
    ...
    def insert_batch_to_postgres(self, records: List[BalanceWithOtherBankRecord], pg_conn):
        """Batch insert balance with other bank records to PostgreSQL"""
        try:
            cursor = pg_conn.cursor()
            
            insert_query = """
            INSERT INTO "balanceWithOtherBanks" (
            ...
            values = [
                (
                    r.reportingDate, r.accountNumber, r.accountName, r.bankCode, r.country,
                    r.relationshipType, r.accountType, r.subAccountType, r.currency,
                    r.orgAmount, r.usdAmount, r.tzsAmount, r.transactionDate, r.pastDueDays,
                    r.allowanceProbableLoss, r.botProvision, r.assetsClassificationCategory,
                    r.contractDate, r.maturityDate, r.externalRatingCorrespondentBank, r.gradesUnratedBanks
                )
                for r in records
            ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Balance with Other Banks Streaming Pipeline')
    ...
    pipeline = BalanceWithOtherBanksStreamingPipeline(batch_size=args.batch_size, consumer_batch_size=args.consumer_batch_size)
