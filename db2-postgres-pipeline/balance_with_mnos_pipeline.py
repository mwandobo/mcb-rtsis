#!/usr/bin/env python3
"""
Balance with MNOs Pipeline - BOT Project
...
class BalanceWithMnosPipeline:
    def __init__(self, limit=None, start_date=None, resume=False):
        ...
        # Get balance with MNOs table config
        self.table_config = self.config.tables.get('balanceWithMnos')
        if not self.table_config:
            raise ValueError("Balance with MNOs table config not found")
        ...
        self.logger.info(f"Balance with MNOs Pipeline initialized")
    ...
    def get_total_count(self):
        """Get total count of balance with MNOs records"""
        try:
    ...
    def run_complete_pipeline(self):
        """Run the complete balance with MNOs pipeline"""
        self.logger.info("Starting Complete Balance with MNOs Pipeline")
        self.logger.info("=" * 60)
        ...
        try:
            ...
            self.logger.info(f"Getting total balance with MNOs records from {self.start_date}...")
            total_records = self.get_total_count()
            self.logger.info(f"Total balance with MNOs records available: {total_records}")
            
            if total_records == 0:
                self.logger.info("No balance with MNOs records found")
                return
            
            # Step 3: Process using the SQL query from config
            self.logger.info("Executing balance with MNOs query...")
            
            with self.get_db2_connection() as conn:
                cursor = conn.cursor()
                
                # Use the query from config as-is (it already has date filter built in)
                query = self.table_config.query
                
                cursor.execute(query)
                rows = cursor.fetchall()
                
                self.logger.info(f"Fetched {len(rows)} balance with MNOs records")
                ...
                # Show sample data
                self.logger.info("Sample balance with MNOs records:")
                for i, row in enumerate(rows[:min(3, len(rows))], 1):
                    self.logger.info(f"  {i}. MNO: {row[2]} | Till: {row[3]} | Currency: {row[4]} | Amount: {row[7]}")
            ...
            with self.get_postgres_connection() as conn:
                ...
                for i, row in enumerate(rows, 1):
                    try:
                        ...
                        record = self.mnos_processor.process_record(row, self.table_config.name)
                        
                        if self.mnos_processor.validate_record(record):
                            # Use upsert with mnoCode as unique constraint (keeps latest balance per MNO)
                            self.mnos_processor.insert_to_postgres(record, pg_cursor)
                            processed_count += 1
                            
                            if processed_count % 100 == 0:
                                self.logger.info(f"Processed {processed_count}/{len(rows)} records...")
                        else:
                            self.logger.warning(f"Invalid balance with MNOs record skipped: {record.till_number}")
                            skipped_count += 1
            ...
            self.logger.info(f"BALANCE WITH MNOS PIPELINE FINISHED!")
            ...
            with self.get_postgres_connection() as conn:
                ...
                cursor.execute(f"""
                    SELECT "mnoCode", "tillNumber", "currency", "orgFloatAmount", "reportingDate"
                ...
                for record in sample_records:
                    mno_code, till_number, currency, amount, reporting_date = record
                    self.logger.info(f"  {mno_code} | {till_number} | {currency} | {amount}")
...
def main():
    ...
    print("Complete Balance with MNOs Pipeline - BOT Project")
    print("=" * 60)
    
    # Enable resume mode by default for balance with MNOs processing - starting from 2024
    pipeline = BalanceWithMnosPipeline(start_date='2024-01-01', resume=True)
