#!/usr/bin/env python3
"""
Outgoing Fund Transfer Streaming Pipeline - Producer and Consumer run simultaneously
Based on outgoing-fund-transfer-v1.sql query
"""
...
class OutgoingFundTransferRecord:
    """Data class for outgoing fund transfer records based on outgoing-fund-transfer-v1.sql"""
    reportingDate: str
    transactionId: str
    transactionDate: str
    transferChannel: str
    ...
    senderIdentificationNumber: str
    recipientName: str
...
class OutgoingFundTransferStreamingPipeline:
    def __init__(self, batch_size=1000, consumer_batch_size=100):
        ...
        self.logger.info("Outgoing Fund Transfer STREAMING Pipeline initialized")
    ...
    def get_outgoing_fund_transfer_query(self):
        """Get the outgoing fund transfer query from outgoing-fund-transfer-v1.sql"""
        sql_file_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            'sqls', 'outgoing-fund-transfer-v1.sql'
        )
    ...
    def get_total_count(self):
        """Get approximate total count of outgoing fund transfer records from DB2"""
        try:
            with self.db2_conn.get_connection(log_connection=False) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM OUTGOING_ORDERS")
                result = cursor.fetchone()
                count = result[0] if result else 0
                self.logger.info(f"Estimated record count from OUTGOING_ORDERS: {count:,}")
                return count
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for outgoing fund transfers with dead-letter exchange"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            # Declare dead-letter exchange and queue for failed messages
            channel.exchange_declare(exchange='outgoing_fund_transfer_dlx', exchange_type='direct', durable=True)
            channel.queue_declare(queue='outgoing_fund_transfer_dead_letter', durable=True)
            channel.queue_bind(
                queue='outgoing_fund_transfer_dead_letter',
                exchange='outgoing_fund_transfer_dlx',
                routing_key='outgoing_fund_transfer_queue'
            )
            
            # Declare main queue with dead-letter exchange routing
            try:
                channel.queue_declare(
                    queue='outgoing_fund_transfer_queue',
                    durable=True,
                    arguments={
                        'x-dead-letter-exchange': 'outgoing_fund_transfer_dlx',
                        'x-dead-letter-routing-key': 'outgoing_fund_transfer_queue'
                    }
                )
                ...
                self.logger.warning(
                    "Queue 'outgoing_fund_transfer_queue' already exists with different args. "
                    "Delete and recreate it to enable dead-letter support."
                )
                connection, channel = self.setup_rabbitmq_connection()
                channel.queue_declare(queue='outgoing_fund_transfer_queue', durable=True)
                self.logger.info("RabbitMQ queue 'outgoing_fund_transfer_queue' setup complete (without DLX)")
    ...
    def process_record(self, row):
        ...
        try:
            ...
            record = OutgoingFundTransferRecord(
                ...
                senderName=safe_string(row[5]),                         # ISSUER_NAME
                senderAccountNumber=safe_string(row[6]) if row[6] else None,         # ORD_CUST_ACCOUNT
                senderIdentificationType=safe_string(row[7]),           # CASE WHEN LENGTH(ISSUER_ID_NUM) = 20...
                senderIdentificationNumber=safe_string(row[8]),         # ISSUER_ID_NUM
                recipientName=safe_string(row[9]),                      # BENEF_NAME
                recipientMobileNumber=safe_string(row[10]) if row[10] else None,     # BENEF_PHONE
                recipientCountry=safe_string(row[11]),                  # CASE UPPER(TRIM(BENEF_COUNTRY))...
                recipientBankOrFspCode=safe_string(row[12]),            # ACC_WITH_BANK_SWIF
                recipientAccountOrWalletNumber=safe_string(row[13]),    # BENEF_ACCOUNT
                serviceChannel=safe_string(row[14]),                    # 'Mobile banking'
                serviceCategory=safe_string(row[15]),                   # 'Mobile banking'
                serviceSubCategory=safe_string(row[16]),                # 'Inter banking'
                currency=safe_string(row[17]),                          # ORDER_PAYABLE_CUR
                orgAmount=safe_string(row[18]),                         # ORDER_AMOUNT
                usdAmount=safe_string(row[19]),                         # USD conversion with fx rates
                tzsAmount=safe_string(row[20]),                         # TZS conversion with fx rates
                purposes=safe_string(row[21]),                          # 'Salaries and wages'
                senderInstruction=safe_string(row[22]),                 # SPECIAL_TERMS
                transactionPlace=safe_string(row[23])                   # CASE UPPER(TRIM(ISSUER_COUNTRY))...
            )
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total outgoing fund transfer records available: {self.total_available:,} (estimated)")
            ...
            query = self.get_outgoing_fund_transfer_query()
            self.logger.info("Executing outgoing fund transfer query (single execution, streaming results)...")
    ...
    def consumer_thread(self):
        ...
        try:
            ...
            # Batch insert buffer
            insert_buffer: List[OutgoingFundTransferRecord] = []
            ...
            def process_message(ch, method, properties, body):
                ...
                try:
                    ...
                    record = OutgoingFundTransferRecord(**record_data)
            ...
            channel.basic_consume(queue='outgoing_fund_transfer_queue', on_message_callback=process_message)
            ...
            while not self.stop_consumer.is_set():
                try:
                    ...
                    if self.producer_finished.is_set():
                        ...
                        queue_state = channel.queue_declare(queue='outgoing_fund_transfer_queue', durable=True, passive=True)
    ...
    def insert_batch_to_postgres(self, records: List[OutgoingFundTransferRecord], pg_conn):
        """Batch insert outgoing fund transfer records to PostgreSQL with duplicate prevention"""
        try:
            ...
            INSERT INTO "outgoingFundTransfer" (
                ...
                "subCategoryTransferChannel", "senderName", "senderAccountNumber",
                "senderIdentificationType", "senderIdentificationNumber", "recipientName",
                "recipientMobileNumber", "recipientCountry", "recipientBankOrFspCode",
                "recipientAccountOrWalletNumber", "serviceChannel", "serviceCategory",
                "serviceSubCategory", "currency", "orgAmount", "usdAmount", "tzsAmount",
                "purposes", "senderInstruction", "transactionPlace"
            ) VALUES %s
            ...
            values = [
                (
                    ...
                    r.subCategoryTransferChannel, r.senderName, r.senderAccountNumber,
                    r.senderIdentificationType, r.senderIdentificationNumber, r.recipientName,
                    r.recipientMobileNumber, r.recipientCountry, r.recipientBankOrFspCode,
                    r.recipientAccountOrWalletNumber, r.serviceChannel, r.serviceCategory,
                    r.serviceSubCategory, r.currency, r.orgAmount, r.usdAmount, r.tzsAmount,
                    r.purposes, r.senderInstruction, r.transactionPlace
                )
                for r in records
            ]
            ...
            self.logger.error(f"Error batch inserting {len(records)} outgoing fund transfer records: {e}")
            raise
    ...
    def ensure_unique_index(self):
        ...
        try:
            with self.get_postgres_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_outgoingfundtransfer_txn_id_unique
                    ON "outgoingFundTransfer" ("transactionId")
                """)
                conn.commit()
    ...
    def run_streaming_pipeline(self):
        ...
        self.logger.info("Starting Outgoing Fund Transfer STREAMING pipeline...")
        ...
        try:
            ...
            self.logger.info(f"""
            ==========================================
            Outgoing Fund Transfer Pipeline Summary:
            ==========================================
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Outgoing Fund Transfer Streaming Pipeline')
    ...
    # Create pipeline
    pipeline = OutgoingFundTransferStreamingPipeline(batch_size=args.batch_size, consumer_batch_size=args.consumer_batch_size)
