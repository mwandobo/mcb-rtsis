...
class POSStreamingPipeline:
    ...
    def get_pos_query(self, last_pos_number=None):
        ...
        if last_pos_number:
            where_clause = f"WHERE at.FK_USRCODE > '{last_pos_number}'"
        ...
        SELECT VARCHAR_FORMAT(CURRENT_TIMESTAMP, 'DDMMYYYYHHMM')    AS reportingDate,
               ...
               NULL                                                 AS returnDate,
               at.FK_USRCODE                                        AS cursor_pos_number
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for POS"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            channel.queue_declare(queue="pos_queue", durable=True)
            connection.close()
            self.logger.info("RabbitMQ queue 'pos_queue' setup complete")
        except Exception as e:
    ...
    def process_record(self, row):
        ...
        # Check if required fields (region, district, ward) are NULL
        # If so, skip this record silently (LEFT JOIN may not find matching agent)
        region = row_data[9]
        district = row_data[10]
        ward = row_data[11]
        
        if not region or str(region).strip() == '' or str(region).strip().upper() in ('NIL', 'NULL', 'NONE'):
            return None
        
        if not district or str(district).strip() == '' or str(district).strip().upper() in ('NIL', 'NULL', 'NONE'):
            return None
        ...
        return POSRecord(
            ...
            posBranchCode=int(row_data[1]) if row_data[1] else 201,
            posNumber=str(row_data[2]).strip() if row_data[2] else None,
            qrFsrCode=str(row_data[3]).strip() if row_data[3] else None,
            posHolderCategory=str(row_data[4]).strip() if row_data[4] else None,
            posHolderName=str(row_data[5]).strip() if row_data[5] else None,
            posHolderNin=str(row_data[6]).strip() if row_data[6] else None,
            posHolderTin=str(row_data[7]).strip() if row_data[7] else None,
            postalCode=str(row_data[8]).strip() if row_data[8] else None,
            region=str(row_data[9]).strip(),  # NOT NULL - validated above
            district=str(row_data[10]).strip(),  # NOT NULL - validated above
            ward=str(row_data[11]).strip(),  # NOT NULL - validated above
            street=str(row_data[12]).strip() if row_data[12] else None,
            houseNumber=str(row_data[13]).strip() if row_data[13] else None,
            gpsCoordinates=str(row_data[14]).strip() if row_data[14] else None,
            linkedAccount=str(row_data[15]).strip() if row_data[15] else None,
            issueDate=str(row_data[16]) if row_data[16] else None,
            returnDate=str(row_data[17]) if row_data[17] else None,
        )

    def insert_to_postgres(self, record: POSRecord, cursor):
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total POS records available: {self.total_available:,}")
            ...
            while True:
                ...
                for attempt in range(self.max_retries):
                    try:
                        with self.db2_conn.get_connection() as conn:
                            cursor = conn.cursor()
                            batch_query = self.get_pos_query(last_pos_number)
                ...
                batch_published = 0
                for row in rows:
                    last_pos_number = row[-1]  # cursor_pos_number
                ...
                try:
                    ...
                    if self.producer_finished.is_set():
                        method = channel.queue_declare(
                            queue="pos_queue", durable=True, passive=True
                        )
                    ...
                    channel.basic_consume(
                        queue="pos_queue", on_message_callback=process_message
                    )
...
def main():
    ...
    pipeline = POSStreamingPipeline(batch_size=args.batch_size)
