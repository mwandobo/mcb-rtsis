#!/usr/bin/env python3
"""
Card Product Streaming Pipeline - Producer and Consumer run simultaneously
Based on card-product.sql query
"""
...
@dataclass
class CardProductRecord:
    """Data class for card product records based on card-product.sql"""
    reportingDate: str
    binNumber: str
    binNumberStartDate: Optional[str]
    currency: str
    cardType: str
...
class CardProductStreamingPipeline:
    def __init__(self, batch_size=1000, consumer_batch_size=100):
        ...
        self.logger.info("Card Product STREAMING Pipeline initialized")
    ...
    def get_card_product_query(self):
        """Get the card product query from card-product.sql"""
        sql_file_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            'sqls', 'card-product.sql'
        )
    ...
    def get_total_count(self):
        """Get approximate total count of card product records from DB2"""
        try:
            with self.db2_conn.get_connection(log_connection=False) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT COUNT(DISTINCT LEFT(TRIM(ca.FULL_CARD_NO), 6))
                    FROM CMS_CARD ca
    ...
    def setup_rabbitmq_queue(self):
        """Setup RabbitMQ queue for card product with dead-letter exchange"""
        try:
            connection, channel = self.setup_rabbitmq_connection()
            
            # Declare dead-letter exchange and queue for failed messages
            channel.exchange_declare(exchange='card_product_dlx', exchange_type='direct', durable=True)
            channel.queue_declare(queue='card_product_dead_letter', durable=True)
            channel.queue_bind(
                queue='card_product_dead_letter',
                exchange='card_product_dlx',
                routing_key='card_product_queue'
            )
            
            # Declare main queue with dead-letter exchange routing
            try:
                channel.queue_declare(
                    queue='card_product_queue',
                    durable=True,
                    arguments={
                        'x-dead-letter-exchange': 'card_product_dlx',
                        'x-dead-letter-routing-key': 'card_product_queue'
                    }
                )
                self.logger.info("RabbitMQ queues setup complete (main + dead-letter)")
            except Exception:
                self.logger.warning("Queue 'card_product_queue' already exists with different args.")
                connection, channel = self.setup_rabbitmq_connection()
                channel.queue_declare(queue='card_product_queue', durable=True)
                self.logger.info("RabbitMQ queue 'card_product_queue' setup complete (without DLX)")
    ...
    def process_record(self, row):
        """Process a single card product record from DB2"""
        try:
            ...
            record = CardProductRecord(
                reportingDate=safe_string(row[0]),           # VARCHAR_FORMAT(CURRENT_TIMESTAMP, 'DDMMYYYYHHMM')
                binNumber=safe_string(row[1]),               # LEFT(TRIM(ca.FULL_CARD_NO), 6)
                binNumberStartDate=safe_string(row[2]) if row[2] else None,  # MIN date
                currency=safe_string(row[3]),                # 'TZS'
                cardType=safe_string(row[4]),                # CASE gd.SERIAL_NUM
                cardTypeSubCategory=safe_string(row[5]) if row[5] else None,  # CASE gd.SERIAL_NUM
                cardSchemeName=safe_string(row[6]),          # 'VISA'
                cardIssuerCategory=safe_string(row[7]),      # 'Domestic'
            ...
            self.logger.error(f"Error processing card product record: {e}")
            self.logger.error(f"Row data: {row}")
            self.logger.error(f"Row length: {len(row)}")
            raise
    
    def validate_record(self, record):
        """Validate card product record"""
        try:
            if not record.binNumber:
                self.logger.warning("Missing binNumber")
                return False
            
            return True
            
        except Exception as e:
            self.logger.error(f"Error validating card product record: {e}")
            return False
    ...
    def producer_thread(self):
        ...
        try:
            ...
            self.logger.info(f"Total card product records available: {self.total_available:,} (estimated)")
            ...
            query = self.get_card_product_query()
            self.logger.info("Executing card product query (single execution, streaming results)...")
    ...
    def consumer_thread(self):
        ...
        try:
            ...
            # Batch insert buffer
            insert_buffer: List[CardProductRecord] = []
            ...
            def process_message(ch, method, properties, body):
                ...
                try:
                    ...
                    record = CardProductRecord(**record_data)
            ...
            channel.basic_consume(queue='card_product_queue', on_message_callback=process_message)
            ...
            while not self.stop_consumer.is_set():
                try:
                    ...
                    if self.producer_finished.is_set():
                        ...
                        queue_state = channel.queue_declare(queue='card_product_queue', durable=True, passive=True)
    ...
    def insert_batch_to_postgres(self, records: List[CardProductRecord], pg_conn):
        """Batch insert card product records to PostgreSQL with duplicate prevention"""
        try:
            cursor = pg_conn.cursor()
            
            insert_query = """
            INSERT INTO "cardProduct" (
                "reportingDate", "binNumber", "binNumberStartDate", currency,
                "cardType", "cardTypeSubCategory", "cardSchemeName", "cardIssuerCategory", "cardIssuer"
            ) VALUES %s
            ON CONFLICT ("binNumber", "cardType") DO NOTHING
            """
            
            values = [
                (
                    r.reportingDate, r.binNumber, r.binNumberStartDate, r.currency,
                    r.cardType, r.cardTypeSubCategory, r.cardSchemeName, r.cardIssuerCategory, r.cardIssuer
                )
                for r in records
            ]
            ...
            self.logger.error(f"Error batch inserting {len(records)} card product records: {e}")
            raise
    
    def ensure_unique_index(self):
        """Ensure unique index on binNumber and cardType exists for ON CONFLICT duplicate prevention"""
        try:
            with self.get_postgres_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_card_product_unique
                    ON "cardProduct"("binNumber", "cardType")
                """)
                conn.commit()
                self.logger.info("Unique index on binNumber, cardType verified/created")
        except Exception as e:
            self.logger.error(f"Failed to create unique index on cardProduct: {e}")
            raise
    
    def run_streaming_pipeline(self):
        """Run the streaming pipeline with simultaneous producer and consumer"""
        self.logger.info("Starting Card Product STREAMING pipeline...")
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info("Card Product STREAMING Pipeline Complete!")
            self.logger.info("=" * 60)
