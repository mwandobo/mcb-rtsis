WITH LatestInstallments AS (SELECT *
     ...
     MainQuery AS (SELECT *
                   FROM (SELECT VARCHAR_FORMAT(CURRENT_TIMESTAMP, 'DDMMYYYYHHMM')                            AS reportingDate,
                                ...
                                CASE
                                    WHEN p.DESCRIPTION LIKE '%BUSINESS%' AND p.DESCRIPTION LIKE '%LOAN%'
                                        THEN 'Business Loan'
                                    WHEN p.DESCRIPTION LIKE '%INSURANCE%' AND p.DESCRIPTION LIKE '%LOAN%'
                                        THEN 'Business Loan'
                                    WHEN p.DESCRIPTION LIKE '%IPF%' AND p.DESCRIPTION LIKE '%LOAN%' THEN 'Business Loan'
                                    WHEN p.DESCRIPTION LIKE '%KILIMO%' AND p.DESCRIPTION LIKE '%LOAN%'
                                        THEN 'Business Loan'
                                    WHEN p.DESCRIPTION LIKE '%MICRO%' AND p.DESCRIPTION LIKE '%LOAN%'
                                        THEN 'Business Loan'
                                    WHEN p.DESCRIPTION LIKE '%UNSECURED%' AND p.DESCRIPTION LIKE '%LOAN%'
                                        THEN 'Business Loan'
                                ...
                                DECIMAL(la.MORATOR_NRM_RATE, 18, 2)                                          AS primeLendingRate,
                                'Fixed'                                                                      AS interestPricingMethod,
                                la.TOT_NRM_INT_AMN                                                           AS annualInterestRate,
                                NULL                                                                         AS effectiveAnnualInterestRate,
                                VARCHAR_FORMAT(la.INSTALL_FIRST_DT, 'DDMMYYYYHHMM')                          AS firstInstallmentPaymentDate,
                                VARCHAR_FORMAT(li.RQ_LST_PAYMENT_DT, 'DDMMYYYYHHMM')                         AS lastPaymentDate,
                                ca.collateralPledged,
                                'Not-restructured'                                                           AS loanFlagType,
                                NULL                                                                         AS restructuringDate,
                                COALESCE(li.RQ_OVERDUE_DAYS, 0)                                              AS pastDueDays,
                                la.OV_CAP_BAL                                                                AS pastDueAmount,
                                'N/A'                                                                        AS internalRiskGroup,
                                ...
                                CASE
                                    ...
                                    ELSE (la.NRM_ACR_INT_BAL + la.OV_ACR_NRM_INT_BAL + la.OV_ACR_PNL_INT_BAL) /
                                         fx.RATE END                                                         AS usdAccruedInterestAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN DECIMAL(
                                            (la.NRM_ACR_INT_BAL + la.OV_ACR_NRM_INT_BAL + la.OV_ACR_PNL_INT_BAL) *
                                            fx.RATE, 18, 2)
                                    ELSE DECIMAL((la.NRM_ACR_INT_BAL + la.OV_ACR_NRM_INT_BAL + la.OV_ACR_PNL_INT_BAL),
                                                 18,
                                                 2) END                                                      AS tzsAccruedInterestAmount,
                                ...
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN DECIMAL(
                                            (la.OV_RL_PNL_INT_BAL + la.OV_URL_PNL_INT_BAL) * fx.RATE, 18, 2)
                                    ELSE DECIMAL((la.OV_RL_PNL_INT_BAL + la.OV_URL_PNL_INT_BAL), 18, 2) END  AS tzsPenaltyChargedAmount,
                                COALESCE(la.TOT_PNL_INT_AMN, 0)                                              AS orgPenaltyPaidAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN COALESCE(la.TOT_PNL_INT_AMN, 0)
                                    ELSE DECIMAL(COALESCE(la.TOT_PNL_INT_AMN, 0) / fx.RATE, 18, 2) END       AS usdPenaltyPaidAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD'
                                        THEN DECIMAL(COALESCE(la.TOT_PNL_INT_AMN, 0) * fx.RATE, 18, 2)
                                    ELSE DECIMAL(COALESCE(la.TOT_PNL_INT_AMN, 0), 18, 2) END                 AS tzsPenaltyPaidAmount,
                                ...
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN DECIMAL(la.TOT_COMMISSION_AMN * fx.RATE, 18, 2)
                                    ELSE DECIMAL(la.TOT_COMMISSION_AMN, 18, 2) END                           AS tzsLoanFeesChargedAmount,
                                la.TOT_EXPENSE_AMN                                                           AS orgLoanFeesPaidAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN la.TOT_EXPENSE_AMN
                                    ELSE DECIMAL(la.TOT_EXPENSE_AMN / fx.RATE, 18, 2) END                    AS usdLoanFeesPaidAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN DECIMAL(la.TOT_EXPENSE_AMN * fx.RATE, 18, 2)
                                    ELSE DECIMAL(la.TOT_EXPENSE_AMN, 18, 2) END                              AS tzsLoanFeesPaidAmount,
                                li.INSTALL_AMN                                                               AS orgTotMonthlyPaymentAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN li.INSTALL_AMN
                                    ELSE DECIMAL(li.INSTALL_AMN / fx.RATE, 18, 2) END                        AS usdTotMonthlyPaymentAmount,
                                CASE
                                    WHEN cu.SHORT_DESCR = 'USD' THEN DECIMAL(li.INSTALL_AMN * fx.RATE, 18, 2)
                                    ELSE DECIMAL(li.INSTALL_AMN, 18, 2) END                                  AS tzsTotMonthlyPaymentAmount,
                                'Households'                                                                 AS sectorSnaClassification,
                                'Current'                                                                    AS assetClassificationCategory,
                                'NoNegativeStatus'                                                           AS negStatusContract,
                                'Personal Loan'                                                              AS customerRole,
                         ...
                         FROM LOAN_ACCOUNT la
                                  ...
                                  LEFT JOIN LOAN_ACCOUNT_INFO lai ON la.FK_UNITCODE = lai.FK_LOAN_ACCOUNTFK AND
                                                                     la.ACC_TYPE = lai.FK0LOAN_ACCOUNTACC AND
                                                                     la.ACC_SN = lai.FK_LOAN_ACCOUNTACC
                                  ...
                                  LEFT JOIN GENERIC_DETAIL id_country
                                            ON id.fkgh_has_been_issu = id_country.fk_generic_headpar AND
                                               id.fkgd_has_been_issu = id_country.serial_num
                                  LEFT JOIN COUNTRIES_LOOKUP cl ON cl.COUNTRY_NAME = id_country.description
                                  LEFT JOIN GENERIC_DETAIL GG ON GG.FK_GENERIC_HEADPAR = la.FKGH_HAS_AS_LOAN_P AND
                                                                 GG.SERIAL_NUM = la.FKGD_HAS_AS_LOAN_P
                                  LEFT JOIN PRODUCT p ON la.FK_LOANFK_PRODUCTI = p.ID_PRODUCT) mq)
SELECT *
FROM (SELECT mq.*,
             ROW_NUMBER() OVER (PARTITION BY mq.loanNumber ORDER BY mq.customerIdentificationNumber) AS rn
      FROM MainQuery mq) t
WHERE t.rn = 1
  AND LENGTH(TRIM(TRANSLATE(t.loanOfficer, '', '0123456789'))) > 0
  AND LENGTH(TRIM(TRANSLATE(t.loanSupervisor, '', '0123456789'))) > 0
