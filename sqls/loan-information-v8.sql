WITH LatestInstallments AS (SELECT *
     ...
     MainQuery AS (SELECT VARCHAR_FORMAT(CURRENT_TIMESTAMP, 'DDMMYYYYHHMM')                     AS reportingDate,
                          ...
                          -- FIX 1: Full name concatenation (FIRST + MIDDLE + SURNAME)
                          TRIM(
                                  COALESCE(TRIM(cust.FIRST_NAME), '') ||
                                  CASE
                                      ...
                                      END ||
                                  CASE
                                      WHEN COALESCE(cust.SURNAME, '') <> ''
                                          THEN ' ' || TRIM(cust.SURNAME)
                                      ELSE ''
                                      END
                          )                                                                     AS clientName,
                          -- FIX 2: Default to Tanzania when country not resolved
                          COALESCE(
                                  CASE WHEN cl.COUNTRY_CODE = 'TZ' THEN 'TANZANIA, UNITED REPUBLIC OF' END,
                          ...
                          -- FIX 3: COALESCE fallback for gender
                          ...
                          -- FIX 4: interestPricingMethod — defaulting to 'Fixed' as a reasonable value
                          'Fixed'                                                               AS interestPricingMethod,
                          la.TOT_NRM_INT_AMN                                                    AS annualInterestRate,
                          NULL                                                                  AS effectiveAnnualInterestRate,
                          -- FIX 5: firstInstallmentPaymentDate formatted as DDMMYYYYHHMM
                          ...
                          -- FIX 6: loanFlagType conditional instead of hardcoded 'Restructured'
                          CASE
                              WHEN la.OV_EXP_DT IS NOT NULL THEN 'Restructured'
                              ELSE 'NotRestructured'
                              END                                                               AS loanFlagType,
                          -- FIX 7: restructuringDate only populated when restructured
                          CASE
                              WHEN la.OV_EXP_DT IS NOT NULL
                                  THEN VARCHAR_FORMAT(la.OV_EXP_DT, 'DDMMYYYYHHMM')
                              ELSE NULL
                          ...
                          CASE
                              WHEN cu.SHORT_DESCR = 'USD' THEN DECIMAL(COALESCE(la.TOT_PNL_INT_AMN, 0) * fx.RATE, 18, 2)
                   ...
                   FROM LOAN_ACCOUNT la
                            ...
                            LEFT JOIN (SELECT *
                                       FROM (SELECT pa.*,
                                             ...
                                             WHERE PRFT_SYSTEM = 4)
                                       WHERE rn = 1) pa ON pa.CUST_ID = la.CUST_ID
