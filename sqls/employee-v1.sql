WITH salary_cte AS (
    SELECT
        STAFF_NO,
        -- Random salary between 400,000 and 3,000,000, rounded to nearest 10,000
        INT(ROUND((RAND() * (3000000 - 400000) + 400000) / 10000.0, 0) * 10000) AS basicSalary
    FROM BANKEMPLOYEE
...
--        null                                                                                         AS psssf
       sc.basicSalary,
       INT(sc.basicSalary * 0.10) AS nhif,
       INT(sc.basicSalary * 0.15) AS psssf
FROM BANKEMPLOYEE be
         ...
         LEFT JOIN salary_cte sc
                   ON be.STAFF_NO = sc.STAFF_NO
