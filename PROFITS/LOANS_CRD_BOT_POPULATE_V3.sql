CREATE PROCEDURE PROFITS.LOANS_CRD_BOT_POPULATE_V3 ( )
...
BEGIN
	DELETE
FROM
	BOT_46_SECONDARYSUBJECT ;
DELETE
FROM
	BOT_25_SECONDARYSUBJECT ;
DELETE
FROM
	BOT_24_PRIMARYSUBJECT ;
DELETE
FROM
	BOT_22_SUBJECTRELATIONS ;
DELETE
FROM
	BOT_41_NONINSTCONNSUBJ ;
DELETE
FROM
	BOT_40_NONINSTCOLLATERAL ;
DELETE
FROM
	BOT_21_NONINSTALMENT ;
DELETE
FROM
	BOT_39_INVBILLCONNSUBJ ;
DELETE
FROM
	BOT_38_INVBILLCOLLATERAL ;
DELETE
FROM
	BOT_19_INVOICEBILL ;
DELETE
...
DELETE
FROM
	BOT_27_CCARDCONNSUBJECT ;
DELETE
FROM
	BOT_31_CCARDCOLLATERAL ;
DELETE
FROM
	BOT_8_CREDITCARD ;
DELETE
...
DELETE
FROM
	BOT_7_STSUBJECTRELATIONS ;
DELETE
FROM
	BOT_6_STORNONINSTALMENT ;
DELETE
FROM
	BOT_5_STORINVOICEBILL ;
DELETE
FROM
	BOT_3_STORCREDITCARD ;
DELETE
FROM
	BOT_1_COMMAND ;
DELETE
...
DELETE
FROM
	LNS_CRD_CUST_SUB_DATA;
DELETE
FROM
	LNS_CRD_COLL_SUB_DATA;
DELETE
FROM
	LNS_CRD_CUST_DATA;
DELETE
FROM
	LNS_CRD_CUST_ADDRESS;
DELETE
FROM
	LNS_CRD_CUST_RELAT;
INSERT
...
SELECT
	...
	SELECT
		B.SCHEDULED_DATE
	FROM
		BANK_PARAMETERS B) AS REPORTING_DATE,
	...
	FROM
		GENERIC_DETAIL D
	WHERE
		D.SERIAL_NUM = L.FKGD_HAS_AS_LOAN_P
		AND D.FK_GENERIC_HEADPAR = L.FKGH_HAS_AS_LOAN_P) AS LOAN_PURPOSE,
	(
	SELECT
		H.SHORT_DESCR
	FROM
		CURRENCY H
	WHERE
		L.FKCUR_IS_MOVED_IN = H.ID_CURRENCY) AS LOAN_CURRENCY,
	'1' AS LOAN_PHASE ,
	...
	CASE
		WHEN F.FKGD_HAS_ADJUST <> 0 THEN 'TRUE'
		ELSE 'FALSE'
	END AS RECHEDULER_LOAN,
	CASE
		WHEN L.LOAN_STATUS>1 THEN ((
		SELECT
			B.SCHEDULED_DATE
		FROM
			BANK_PARAMETERS B)-L.OV_EXP_DT)
		ELSE 0
	...
	FROM
		GENERIC_DETAIL D
	WHERE
		D.SERIAL_NUM = L.FKGD_HAS_AS_FINANC
		AND D.FK_GENERIC_HEADPAR = L.FKGH_HAS_AS_FINANC ) AS ECONOMIC_SECTOR,
	...
	FROM
		...
		GENERIC_DETAIL T
	WHERE
		W.FK_PRODUCTID_PRODU = L.FK_LOANFK_PRODUCTI
		AND W.FKGD_HAS_ACCOUNT_T = T.SERIAL_NUM
		AND
                             W.FKGH_HAS_ACCOUNT_T = T.FK_GENERIC_HEADPAR) AS LOAN_TYPE,
	L.DRAWDOWN_FST_AMN AS LOAN_AMOUNT,
	CASE
		WHEN L.ACC_MECHANISM IN (4, 5) THEN 'Fixed'
		ELSE 'Variable'
	END AS INSTALLMENT_TYPE,
	CASE
		WHEN L.INSTALL_FREQ = 1 THEN 'MonthlyInstalments30Days'
		...
		ELSE
		CASE
			WHEN L.INSTALL_COUNT = 1 THEN 'AtTheFinalDayOfThePeriodOfContract'
...
FROM
	...
	LOAN_ACCOUNT_INFO F
WHERE
	P.LNS_OPEN_UNIT = L.FK_UNITCODE
	AND P.LNS_SN = L.ACC_SN
	AND P.LNS_TYPE = L.ACC_TYPE
	AND L.ACC_STATUS = '3'
	AND (
	SELECT
		DAYS(CURR_TRX_DATE) - DAYS(L.LST_TRX_DT)
	FROM
		BANK_PARAMETERS)>= 90
...
WHERE
	...
	WHERE
		...
		SELECT
			max(E.TMSTAMP)
		FROM
			LOAN_ACCOUNT_EXTRA E
		WHERE
			E.ACC_UNIT = L.FK_UNITCODE
...
SELECT
	...
	C.CUST_TYPE ,
	CASE
		WHEN (C.BUSINESS_IND = '1'
			AND C.CUST_TYPE = '1') THEN 'SoleProprietor'
		WHEN (C.CUST_TYPE = '1'
			AND C.BUSINESS_IND <> '1') THEN 'Individual'
		ELSE ''
	END ,
	TRIM(C.FIRST_NAME) ,
	TRIM(C.MIDDLE_NAME) ,
	...
	'',
	CASE
		WHEN C.BUSINESS_IND = '1' THEN C.SECOND_SURNAME
		ELSE ''
	END,
	C.MOBILE_TEL ,
	C.TELEPHONE_1,
...
WHERE
	...
	FROM
		LOAN_ACCOUNT LA
	WHERE
		LA.CUST_ID = C.CUST_ID
		AND (LA.ACC_STATUS <> '3'
			OR (LA.ACC_STATUS = '3'
				AND (
				SELECT
					DAYS(CURR_TRX_DATE) - DAYS(LA.LST_TRX_DT)
				FROM
					BANK_PARAMETERS)<90 )));
--  KAposto 01/02/2023
 
UPDATE
	LNS_CRD_CUST_DATA LC
SET
	LC.MARITAL_STS = TRIM(NVL((SELECT TRIM(GD.DESCRIPTION)
...
CC.FK_GENERIC_DETASER = GD.SERIAL_NUM AND LC.cust_id = CC.FK_CUSTOMERCUST_ID ), 'SINGLE'))
WHERE
	LC.cust_type = '1';
UPDATE
	LNS_CRD_CUST_DATA LC
SET
	LC.MARITAL_STS = 'OTHER'
WHERE
	LC.cust_type = '2';
UPDATE
	LNS_CRD_CUST_data lc
SET
	...
	FROM
		...
		generic_Detail gd
	WHERE
		CC.FK_CATEGORYCATEGOR = 'NATIONAL'
		AND CC.FK_GENERIC_DETAFK = GD.FK_GENERIC_HEADPAR
		AND
CC.FK_GENERIC_DETASER = GD.SERIAL_NUM
		AND LC.cust_id = CC.FK_CUSTOMERCUST_ID )
WHERE
	LC.cust_type = '1';
UPDATE
	LNS_CRD_CUST_data lc
...
(
	SELECT
		CI.IND_MAIN_INCOME / 12,
		CR.SHORT_DESCR,
		CI.BK_COST_OF_SELLING / 12 ,
		CR.SHORT_DESCR
	FROM
		CUST_IMAGE CI,
		CURRENCY CR
	WHERE
		CI.FK_CUSTOMERCUST_ID = LC.cust_id
		AND CI.FK_CURRENCYID_CURR = CR.ID_CURRENCY )
WHERE
	LC.cust_type = '1';
UPDATE
	LNS_CRD_CUST_DATA LC
SET
	...
	FROM
		...
		GENERIC_DETAIL GD
	WHERE
		CC.FK_CATEGORYCATEGOR = 'EDULEVEL'
		AND CC.FK_GENERIC_DETAFK = GD.FK_GENERIC_HEADPAR
		AND
CC.FK_GENERIC_DETASER = GD.SERIAL_NUM
		AND LC.CUST_ID = CC.FK_CUSTOMERCUST_ID )
...
SET
	LC.country_of_birth = (
	SELECT
		TRIM(GD.SHORT_DESCRIPTION)
	FROM
		customer_category cc,
		generic_Detail gd
	WHERE
		CC.FK_CATEGORYCATEGOR = 'BCOUNTRY'
		AND CC.FK_GENERIC_DETAFK = GD.FK_GENERIC_HEADPAR
		AND
CC.FK_GENERIC_DETASER = GD.SERIAL_NUM
		AND LC.cust_id = CC.FK_CUSTOMERCUST_ID )
...
SET
	...
	SELECT
		...
		(
		SELECT
			TRIM(GD1.DESCRIPTION)
		FROM
			GENERIC_DETAIL GD1
		WHERE
			GD1.FK_GENERIC_HEADPAR = OI.FKGH_HAS_BEEN_ISSU
	...
	FROM
		...
		GENERIC_DETAIL GD
	WHERE
		OI.FK_CUSTOMERCUST_ID = LC.cust_id
...
SET
	...
	LC.wi_issue_auth )=
      (
	SELECT
		CHAR(REPLACE(OI.ID_NO, '-', ''), 16),
		(CASE
			WHEN OI.EXPIRY_DATE = TO_DATE('01-01-0001', 'DD-MM-YYYY') THEN NULL
			ELSE OI.EXPIRY_DATE
		END),
		(CASE
			WHEN OI.ISSUE_DATE = TO_DATE('01-01-0001', 'DD-MM-YYYY') THEN NULL
			ELSE OI.ISSUE_DATE
		END),
		(
		SELECT
...
SET
	...
	FROM
		...
		generic_Detail gd
	WHERE
		CC.FK_CATEGORYCATEGOR = 'LAWSHAPE'
		AND CC.FK_GENERIC_DETAFK = GD.FK_GENERIC_HEADPAR
		AND
CC.FK_GENERIC_DETASER = GD.SERIAL_NUM
		AND LC.cust_id = CC.FK_CUSTOMERCUST_ID )
...
SET
	LC.tax_id = (
	SELECT
		CONCAT((CONCAT((CONCAT(SUBSTR(TRIM(OA.AFM_NO), 1, 3), '-')),((CONCAT(SUBSTR(TRIM(OA.AFM_NO), 4, 3), '-'))))), SUBSTR(TRIM(OA.AFM_NO), 7, 3))
	FROM
		OTHER_AFM OA
	WHERE
		OA.FK_CUSTOMERCUST_ID = LC.cust_id
		AND OA.MAIN_FLAG = '1')
WHERE
	LC.cust_type = '2'
	OR (LC.cust_type = '1'
		AND LC.IND_CLASS = 'SoleProprietor');
INSERT
	INTO
	LNS_CRD_CUST_SUB_DATA
(REPORTING_DATE,
...
FROM
  ...
  ,
	customer c
WHERE
	LC.CUST_ID = CA.FK_CUSTOMERCUST_ID
	...
	SELECT
		*
	FROM
		LNS_CRD_CUST_ADDRESS L
	WHERE
		L.CUST_ID = C.CUST_ID
		AND L.ADDRESS_TYPE = 'Physical' )
...
WHERE
	...
	WHERE
		L.CUST_ID = C.CUST_ID
		AND L.ADDRESS_TYPE = 'Registration' )
	AND GD.FK_GENERIC_HEADPAR = CA.FKGH_HAS_COUNTRY
	AND GD.SERIAL_NUM = CA.FKGD_HAS_COUNTRY
	AND GD1.FK_GENERIC_HEADPAR = CA.FKGH_HAS_AS_DISTRI
	AND GD1.SERIAL_NUM = CA.FKGD_HAS_AS_DISTRI
...
'Other'
	...
	FK_CUSTOMERCUST_ID
FROM
	ACCOUNT_COLLATERAL AC,
	LNS_CRD_LOAN_DATA J
WHERE
	AC.PRFT_ACCOUNT = J.LOAN_CODE
...
INSERT
  ...
  SELECT
	CA.FK_CUSTOMERCUST_ID,
	CASE
		WHEN ( CA.ADDRESS_TYPE = '2'
			AND LC.CUST_TYPE = '1') THEN 'Permanent residence'
		WHEN ( CA.ADDRESS_TYPE = '3'
			AND LC.CUST_TYPE = '1') THEN 'Physical'
		WHEN ( CA.ADDRESS_TYPE = '4'
			AND LC.CUST_TYPE = '1') THEN 'Employer'
		WHEN ( CA.ADDRESS_TYPE = '1'
			AND LC.CUST_TYPE = '2') THEN 'Registration'
		WHEN ( CA.ADDRESS_TYPE = '4'
			AND LC.CUST_TYPE = '2') THEN 'Business'
	END ,
	TRIM(GD.SHORT_DESCRIPTION) ,
	TRIM(GD1.DESCRIPTION),
	TRIM (CA.ADDRESS_6),
...
UPDATE
	LNS_CRD_CUST_DATA
SET
	CELLULAR_PHONE = CONCAT(0, substr(CELLULAR_PHONE, 5, 12))
WHERE
	substr(CELLULAR_PHONE, 1, 4)= '+255';
UPDATE
	LNS_CRD_CUST_DATA
SET
	CELLULAR_PHONE = CONCAT(0, substr(CELLULAR_PHONE, 4, 12))
WHERE
	substr(CELLULAR_PHONE, 1, 3)= '255';
UPDATE
	LNS_CRD_CUST_DATA
SET
	CELLULAR_PHONE = CONCAT(0, substr(FIXED_LINE, 5, 12))
WHERE
	substr(FIXED_LINE, 1, 4)= '+255';
UPDATE
	LNS_CRD_CUST_DATA
SET
	CELLULAR_PHONE = CONCAT(0, substr(FIXED_LINE, 4, 12))
WHERE
	substr(FIXED_LINE, 1, 3)= '255';
