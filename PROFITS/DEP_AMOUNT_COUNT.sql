...
WITH config1
     AS (  SELECT DISTINCT 
          ...
          ,oneacc
           AS (SELECT   DISTINCT count(*)
/*                                                 (
                           NVL (
                                 (
                                   CASE
                                      WHEN amt_type = 'GROSS AMOUNT'        THEN ((BOOK_BALANCE +ACCR_CR_INTEREST)* fixing_rate * factor1)
                                      WHEN amt_type = 'FINAL INTEREST'      THEN (RATE)*1000
                                      WHEN amt_type = 'FINAL INTEREST AMT'  THEN (ACCR_CR_INTEREST*fixing_rate* factor1)
                                      WHEN amt_type = 'NO OF ACCOUNTS'      THEN count(*)*1000                                           
                                      WHEN amt_type = 'BOOK BALANCE'        THEN BOOK_BALANCE*fixing_rate*factor1
                                      WHEN amt_type = 'INTEREST PAYABLE'    THEN ACCR_CR_INTEREST * fixing_rate * factor1
...
--                                      WHEN amt_type = 'RATE MAX'            THEN MAX(RATE)*1000                                         
                                      WHEN amt_type = 'WEIGHTED AVERAGE'    THEN
                                                                                (CASE WHEN BOOK_BALANCE <> 0 THEN
                                                                                        ((((BOOK_BALANCE +ACCR_CR_INTEREST) * fixing_rate)* RATE* factor1)
                                                                                     /  ((BOOK_BALANCE +ACCR_CR_INTEREST) * fixing_rate * factor1))*1000
                                                                                     ELSE BOOK_BALANCE*1000
                                                                                END)
                                   END)
                              / 1000
                             ,0)
...
FROM     eom_deposits, config1
               WHERE     eom_deposits.entry_status NOT IN ('0', '3', '4')
...
--                                (config1.currency) = 'FC'  AND config1.CURRENCY_TYPE = 'USD' /* and eom_deposits.id_currency = 1*/ or
                        ...
                        (CASE
                             ...
                             WHEN config1.date_from = 'NOLIMIT'                     THEN TO_DATE ('01/02/1970','DD/MM/YYYY') 
...
amt_type,
      ...
      SELECT distinct NVL (SUM (NVL (oneacc0.amount, 0) + NVL (acc0.amount, 0)), 0) AS amount
      FROM   oneacc0, acc0
DO
