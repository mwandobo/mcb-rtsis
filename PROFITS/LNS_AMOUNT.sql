     ...
     AS (SELECT   DISTINCT
                   (
                     NVL (
                           (
                             CASE
                              when amt_type = 'NUMBER OF BORROWERS'  then count(*)*1000
                                WHEN amt_type = 'GROSS AMOUNT'
                                THEN
                                   (gross_total * fixing_rate * factor1)
                                WHEN amt_type = 'PRINCIPAL AMOUNT'
                                THEN
                                   (1 * fixing_rate * factor1)
                                WHEN amt_type = 'DISBURSMENT AMOUNT'
                                THEN
                                   (tot_drawdown_amn * fixing_rate * factor1)
                                WHEN amt_type = 'WRITE OFF PAYMENTS'
                                THEN
                                     (  loan0.capital_cr
                                      + loan0.rl_int_cr
                                      + loan0.rl_pnl_int_cr)
                                   * fixing_rate
                                   * factor1
                                WHEN amt_type = 'ANNUAL WRITTEN OFF'
                                THEN
                                     (  loan0.capital_db
                                      + loan0.rl_int_db
                                      + loan0.rl_pnl_int_db
                                   ...
                                   * fixing_rate
                                   * factor1
                                WHEN amt_type = 'FINAL INTEREST'
                                THEN
                                   (final_interest * fixing_rate) * factor1
                                WHEN amt_type = 'PROVISION AMOUNT'
                                THEN
                                   (gross_total * fixing_rate * factor1)
                                WHEN amt_type = 'WEIGHTED AVERAGE'
--                                THEN (case when gross_total = 0 then 0 else  (  (gross_total * fixing_rate) * final_interest * factor1) / (gross_total * fixing_rate * factor1) end)
                                THEN (case when gross_total = 0 then 0 else   (  
                                                                                sum(nvl(gross_total,0)* fixing_rate* nvl(final_interest,0)) /
                                                                                sum(nvl(gross_total,0)* fixing_rate)
                                                                                )
                                                                                end)*100
                               END)
                        / 1000
         ...
         WHERE        eom_loans.id_product BETWEEN config1.product_from
                  ...
                  AND eom_date BETWEEN (CASE
                                   ...
                                   AND (CASE
                                        ...
                                        /* 31st Dec, Previous Year */
                                        END)
         GROUP BY config1.amt_type,gross_total , fixing_rate , factor1,tot_drawdown_amn,
loan0.capital_db,
loan0.rl_int_db,
loan0.rl_pnl_int_db,
loan0.rl_int_tr,
