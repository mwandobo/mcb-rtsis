           ...
           AS (SELECT   DISTINCT
               ...
               WHERE     eom_deposits.entry_status NOT IN ('0', '3', '4')
                        ...
                        (CASE
                             ...
                             WHEN config1.date_from = 'NOLIMIT'                     THEN TO_DATE ('01/02/2017','DD/MM/YYYY') 
...
min0 as(
      SELECT max(oneacc0.amount) as rate
      FROM   oneacc0
      union select max(acc0.amount)  as rate
      FROM   acc0)
      select max(rate) as amount
      from min0
DO
