...
WITH config1
    ...
    ,main0
     AS (SELECT   DISTINCT 
                        sum(nvl(gross_total,0)* fixing_rate* nvl(final_interest,0)) /   sum(nvl(gross_total,0)* fixing_rate)                      
     /*        (case when gross_total = 0 then 0 else   (  
