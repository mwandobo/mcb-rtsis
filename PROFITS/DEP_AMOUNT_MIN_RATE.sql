...
min0 as(
      SELECT min(oneacc0.amount) as rate
      FROM   oneacc0
      union select min(acc0.amount)  as rate
      FROM   acc0)
      select min(rate) as amount
      from min0
DO
