     ...
     AS (SELECT   DISTINCT count(*)
                 /*  (
                     NVL (
                           (
                             CASE
                              when amt_type = 'NUMBER OF BORROWERS'  then count(*)
         ...
         WHERE        eom_loans.id_product BETWEEN config1.product_from
                  ...
                  AND eom_date BETWEEN (CASE
                                   ...
                                   AND (CASE
                                        ...
                                        /* 31st Dec, Previous Year */
                                        END)
--         GROUP BY config1.amt_type,gross_total , fixing_rate , factor1,final_interest
         )
    ,main00
